...
利用者の操作画面はすべて業務ポータルとする。共通ヘッダーから「ダッシュボード」「案件一覧」「資料検索」を切り替える。案件一覧は、左の案件一覧、中央のタブ型ワークスペース、右の案件内AIチャットを同時に表示する三分割画面とし、案件選択に連動して中央と右の対象案件を切り替える。Open WebUIの画面へ遷移せず、ポータルが公開APIを通じてRAGチャットを表示する。

### 画面の分担

| 画面/操作 | 主な利用者操作 | 実装先 |
| --- | --- | --- |
| ダッシュボード | 全体概要、案件選択、最近の資料、選択案件の詳細確認 | 業務ポータル |
| 左: 案件一覧 | 案件検索・状態絞込み、案件選択、状態・分野・所在地・未処理資料・TODOの確認 | 業務ポータル |
| 中央: タブ型ワークスペース | 概要、資料、CSVデータ、資料メモ、コメント、FAQ、メンバー、成果物・ジョブ | 業務ポータル + 業務API |
| 右: 案件内チャット | スレッド切替、新規チャット、資料質問、回答モード、根拠表示 | 業務ポータル + 業務API → Open WebUI公開API |
| 資料検索 | 案件横断のキーワード・条件検索、位置情報、結果一覧、資料プレビュー | 業務ポータル + 業務API |

### 最小導線

```text
案件一覧を選択
  → 中央: 選択案件のタブ型ワークスペースを更新
  → 右: 同じ案件のスレッドとAIチャットを更新
```

1. 利用者が左の案件一覧から案件を選ぶと、中央タブと右チャットを同じ案件へ切り替える。
2. 利用者が中央の資料タブから資料をアップロードすると、業務APIがOpen WebUIへファイルを登録する。RAG処理完了後に案件専用Knowledgeへ追加し、ポータルで同期状態を表示する。
3. 利用者は右のチャットパネルで新規チャットを作成し、案件専用Knowledgeと案件のシステムプロンプトを使ったRAGチャットを開始する。同一案件内の複数スレッドはポータルで管理する。
...
- 回答の根拠資料を選択すると、中央を資料プレビュー、CSVデータ、資料メモの該当タブへ切り替える。PDFページ表示、CSV行ハイライト、Markdown該当箇所表示は後続で実装する。

### 視覚・操作の原則

- **案件優先**: 左の案件一覧を常時表示し、選択案件を明確に強調する。中央と右は必ず同じ案件コンテキストを表示する。
...
- **高密度表示**: 技術者が案件、資料、会話を一画面で確認できる情報密度を優先する。狭い画面では左右パネルを折りたたみ可能にする。
