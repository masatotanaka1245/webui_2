...
利用者の操作画面はすべて業務ポータルとする。ポータルは、左の案件一覧、中央のタブ型ワークスペース、右の案件内AIチャットを同時に表示する三分割画面とし、案件選択に連動して中央と右の対象案件を切り替える。Open WebUIの画面へ遷移せず、ポータルが公開APIを通じてRAGチャットを表示する。

### 画面の分担

| 画面/操作 | 主な利用者操作 | 実装先 |
| --- | --- | --- |
| 左: 案件一覧 | 案件選択、状態・分野・所在地・未処理資料・TODOの確認 | 業務ポータル |
